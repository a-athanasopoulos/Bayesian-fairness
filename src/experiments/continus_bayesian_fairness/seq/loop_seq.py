...
def run_marginal_algorithm_seq(train_data,
    ...
    for step in range(horizon):
        ...
        predictions = policy.predict(train_data.iloc[data_from:data_till][X_atr]) >= 0.5
        accepted_mask[data_from: data_till] = predictions.ravel()
...
def run_bootstrap_algorithm_seq(train_data,
    ...
    for step in range(horizon):
        ...
        print(f"step {step + 1} accepted: ", accepted_numbers[step])
